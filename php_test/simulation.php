...
function createBlessing(string $blessingId): ?Blessing {
    ...
    $blessingFile = __DIR__ . '/classes/blessings/' . $blessingId . '.php';
...
<body>
    ...
    <script>
        ...
        <?php if ($blessingResults): ?>
        document.querySelector('[data-tab="blessing"]').click();
        <?php endif; ?>
        
        // Mise à jour du total de combats en temps réel (classique)
        function updateClassicEstimate() {
            const combats = parseInt(document.getElementById('combats_count').value) || 0;
            const matchups = <?php echo $nbMatchups; ?>;
            const totalCombats = combats * matchups;
            document.getElementById('totalCombats').textContent = totalCombats.toLocaleString('fr-FR');
            
            const estimatedSeconds = totalCombats / COMBATS_PER_SECOND_CLASSIC;
            document.getElementById('timeEstimateClassic').textContent = formatTime(estimatedSeconds);
        }
        ...
        function updateBlessingEstimate() {
            ...
            document.getElementById('totalCombatsBlessing').textContent = totalCombats.toLocaleString('fr-FR');
            
            const estimatedSeconds = totalCombats / COMBATS_PER_SECOND_BLESSING;
            document.getElementById('timeEstimateBlessing').textContent = formatTime(estimatedSeconds);
        }
        
        document.getElementById('combats_count').addEventListener('input', updateClassicEstimate);
        
        // Constantes pour le calcul
        const NB_PERSONNAGES = <?php echo count($personnages); ?>;
        const NB_BLESSINGS = <?php echo count(array_filter($blessingsList, fn($b) => $b['id'] !== 'none')); ?>;
        const NB_MATCHUPS_CLASSIC = (NB_PERSONNAGES * (NB_PERSONNAGES - 1)) / 2;
        const COMBATS_PER_SECOND_CLASSIC = 3600 / 3; // 3600 combats en 3 secondes (classique)
        const COMBATS_PER_SECOND_BLESSING = 3600 / 5; // 3600 combats en 5 secondes (bénédictions)
        ...
        function showLoading(totalCombats, isBlessing = false) {
            ...
            combatCountEl.textContent = totalCombats.toLocaleString('fr-FR') + ' combats';
            ...
            timeEstimateEl.textContent = 'Temps estimé : ' + formatTime(estimatedSeconds);
            ...
            loadingInterval = setInterval(() => {
                ...
                subtext.textContent = loadingMessages[messageIndex];
            }, 2000);
        }
        
        // Attacher l'événement au formulaire classique
        document.querySelector('#tab-classic .simulation-form').addEventListener('submit', function(e) {
        ...
        document.querySelector('#tab-blessing .simulation-form').addEventListener('submit', function(e) {
            const totalCombats = calculateBlessingCombats();
            showLoading(totalCombats, true);
        });
        
        // Événements pour mise à jour en temps réel du formulaire bénédictions
        document.getElementById('hero_select').addEventListener('change', updateBlessingEstimate);
        document.getElementById('blessing_select').addEventListener('change', updateBlessingEstimate);
        document.getElementById('opponent_blessing_mode').addEventListener('change', updateBlessingEstimate);
        document.getElementById('combats_count_blessing').addEventListener('input', updateBlessingEstimate);
        
        // Initialiser les estimations au chargement de la page
